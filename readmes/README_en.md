...
Cloudflare Workers Builds can retain its default deployment commands:
...
The build fetches the frontend and runs Wrangler in dry-run mode without provisioning remote resources. An existing `pnpm run build` command also works and builds artifacts for other platforms. Non-production branches upload versions without switching production traffic. A new Worker must complete a production deployment first. See the [Workers Builds configuration](https://developers.cloudflare.com/workers/ci-cd/builds/configuration/).
