...
const NfsConfig: React.FC = () => {
  ...
  const fetchConfig = async () => {
    try {
      const res: any = await apiService.get('/@token/select/none?token_type=nfs');
  ...
  const handleSave = async (values: any) => {
    setLoading(true);
    try {
      const res: any = await apiService.post('/@token/config/none', {
        token_uuid: 'nfs-default',
        token_path: values.export_path || '/',
        token_user: 'system',
        token_type: 'nfs',
        token_info: JSON.stringify(values),
