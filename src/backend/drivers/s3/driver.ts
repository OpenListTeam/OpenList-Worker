...
export class S3Driver implements StorageDriver {
  ...
  private get rootPrefix(): string {
    const root = (this.addition.root_folder_path || "/").replace(/^\/+|\/+$/g, "")
    return root ? `${root}/` : ""
  }
  ...
  async get(_virtualPath: string, physicalPath: string): Promise<FileItem> {
    ...
    if (!s3Key.endsWith("/")) {
      try {
        const dirResult = await this.client.listObjects(`${s3Key}/`, undefined, 1)
        if (dirResult.Contents.length > 0 || dirResult.CommonPrefixes.length > 0) {
          return {
            name,
            size: 0,
            is_dir: true,
  ...
  private async copyDirRecursive(srcPrefix: string, dstPrefix: string): Promise<void> {
  ...
  async move(
    ...
    const dstKey = dstBase.endsWith("/") ? `${dstBase}${name}` : `${dstBase}/${name}`
