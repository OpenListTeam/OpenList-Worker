...
function envNumber(c: any, key: string, fallback: number): number {
  const raw = c.env?.[key] ?? (typeof process !== "undefined" ? process.env?.[key] : undefined)
...
function storageContext(c: any) {
  try {
    ...
    return { waitUntil: (promise: Promise<unknown>) => c.executionCtx.waitUntil(promise) }
  } catch {
    return undefined
  }
}

function errorResponse(c: any, status: 400 | 401 | 403 | 413 | 500 | 501, message: string) {
...
async function readJson(c: any): Promise<any> {
  ...
  if (declared > max) throw new Error(`Seed request exceeds the ${max} byte limit`)
  const text = await c.req.text()
  if (new TextEncoder().encode(text).byteLength > max) throw new Error(`Seed request exceeds the ${max} byte limit`)
  if (!text.trim()) return {}
...
function writableActualPath(user: any, value: unknown): string {
  ...
  if (basePath !== "/" && actualPath !== basePath && !actualPath.startsWith(`${basePath}/`)) {
...
function contentBytes(value: unknown, encoding?: unknown): Uint8Array {
  ...
  if (encoding === "text" || value.trimStart().startsWith("{") || value.trimStart().startsWith("d")) {
...
function detectFormat(value: unknown): SeedFormat | undefined {
  ...
  return format === "oss" || format === "torrent" || format === "cas" ? format : undefined
}
...
function encodedResult(seed: SharingSeed, format: SeedFormat, bytes: Uint8Array, extra: Record<string, unknown> = {}) {
...
function allowedHosts(c: any): string[] {
  const raw = c.env?.SEED_SOURCE_ALLOWED_HOSTS ??
    (typeof process !== "undefined" ? process.env?.SEED_SOURCE_ALLOWED_HOSTS : "")
  return String(raw || "").split(/[\s,;]+/).map((host) => host.toLowerCase()).filter(Boolean)
}

function hostAllowed(hostname: string, allowlist: string[]): boolean {
  const host = hostname.toLowerCase()
  return allowlist.some((entry) =>
    entry.startsWith("*.") ? host.endsWith(entry.slice(1)) && host !== entry.slice(2) : host === entry,
  )
}
...
function validateSource(c: any, source: SeedSource): URL {
  ...
  if (url.username || url.password) throw new Error("Seed source URL credentials are not allowed")
  const type = source.type.toLowerCase()
  const sameOpenList =
    url.origin === requestUrl.origin &&
    (/^\/(api\/)?p\//.test(url.pathname) || url.pathname.startsWith("/d/") || url.pathname.startsWith("/@s/"))
  const shareSource = ["share", "openlist-share"].includes(type) && url.pathname.startsWith("/@s/")
  const directSource = ["direct", "openlist-direct"].includes(type) && sameOpenList
  if (!shareSource && !directSource && !hostAllowed(url.hostname, allowedHosts(c))) {
    throw new Error("Seed source host is not an approved OpenList endpoint or configured domain")
  }
  if (source.expires_at) {
    const expires = Date.parse(source.expires_at)
    if (!Number.isFinite(expires) || expires <= Date.now()) throw new Error("Seed source has expired")
  }
  return url
}
  ...
  for (let redirect = 0; redirect <= 3; redirect++) {
    ...
    if (restrictToSeedSources) {
      ...
      const sameOpenList =
        ...
        (/^\/(api\/)?p\//.test(current.pathname) || current.pathname.startsWith("/@s/"))
      if (!sameOpenList && !hostAllowed(current.hostname, allowedHosts(c))) {
        throw new Error("Seed fetch redirect left the approved source allowlist")
      }
    }
    const response = await fetch(current, { ...init, redirect: "manual" })
    if (response.status < 300 || response.status >= 400) return response
    const location = response.headers.get("Location")
    if (!location) throw new Error("Seed fetch redirect is missing a Location header")
    current = new URL(location, current)
  }
  throw new Error("Seed fetch exceeded the redirect limit")
}

async function readStoredSeed(c: any, user: any, path: string): Promise<Uint8Array> {
  const actualPath = getActualPath(user, normalizeVirtualPath(path))
  const { item } = await getItem(actualPath, storageContext(c))
  const max = envNumber(c, "SEED_MAX_METADATA_SIZE", DEFAULT_MAX_SEED_BYTES)
  if (item.is_dir || item.size < 0 || item.size > max) throw new Error(`Seed file exceeds the ${max} byte limit`)
  const rawUrl = item.raw_url || `${new URL(c.req.url).origin}/api/p${actualPath}`
  const headers = item.raw_url_headers || {}
  if (!item.raw_url && c.req.header("Authorization")) headers.Authorization = c.req.header("Authorization")!
  const response = await fetchSafe(c, new URL(rawUrl, c.req.url), { headers })
  if (!response.ok) throw new Error(`Seed file download failed with HTTP ${response.status}`)
...
async function parseBodySeed(c: any, user: any, body: any) {
  ...
  const max = envNumber(c, "SEED_MAX_METADATA_SIZE", DEFAULT_MAX_SEED_BYTES)
  const hasInlineData = body.seed_data || body.content || body.data || body.torrent_data
  const bytes = body.seed_path || (body.path && !hasInlineData)
    ? await readStoredSeed(c, user, String(body.seed_path || body.path))
    : contentBytes(body.seed_data ?? body.content ?? body.data ?? body.torrent_data, body.encoding)
  if (bytes.byteLength > max) throw new Error(`Seed content exceeds the ${max} byte limit`)
  return parseSeed(bytes, detectFormat(body.format || body.from_format || body.file_name?.split(".").pop()))
}
...
async function collectSourceFiles(c: any, user: any, requested: unknown): Promise<SourceFile[]> {
  ...
  const visit = async (virtualPath: string, relativePath: string): Promise<void> => {
    if (result.length >= maxFiles) throw new Error(`Seed file count exceeds the ${maxFiles} file limit`)
    const actualPath = getActualPath(user, virtualPath)
    const { item } = await getItem(actualPath, storageContext(c))
    if (!item.is_dir) {
      const rawUrl = item.raw_url || `${new URL(c.req.url).origin}/api/p${actualPath}`
...
function conversionDiagnostics(seed: SharingSeed, format: SeedFormat): string[] {
  if (format === "cas") {
    if (seed.files.length !== 1) return ["CAS supports exactly one file"]
    const file = seed.files[0]
    const diagnostics: string[] = []
    if (!file.hashes.md5) diagnostics.push(`${file.path}: missing whole-file MD5`)
    const hasLegacyAggregate = !!file.cas_slice_md5
    const hasFixedPieces = seed.piece_size === DEFAULT_PIECE_SIZE && file.hashes.pieces.md5.length > 0
    if (file.size > DEFAULT_PIECE_SIZE && !hasLegacyAggregate && !hasFixedPieces) {
      diagnostics.push(`${file.path}: missing legacy slice MD5 or complete 10 MiB MD5 pieces`)
    }
    return diagnostics
  }
  if (format === "torrent") {
    ...
    seed.files.forEach((file, index) => {
      if (!file.hashes.pieces.sha1.length) diagnostics.push(`${file.path}: missing SHA-1 piece hashes`)
      if (seed.files.length > 1 && index < seed.files.length - 1 && file.size % seed.piece_size !== 0) {
...
function seedConversionStates(seed: SharingSeed) {
  return Object.fromEntries((["oss", "torrent", "cas"] as SeedFormat[]).map((format) => {
    const missing = conversionDiagnostics(seed, format)
    return [format, { feasible: missing.length === 0, missing }]
  }))
}

async function saveEncodedSeed(c: any, user: any, path: unknown, bytes: Uint8Array): Promise<void> {
  if (!path) return
...
async function generateSeed(
  ...
  const pieceSize = formats.includes("cas") ? DEFAULT_PIECE_SIZE : Number(body.piece_size || DEFAULT_PIECE_SIZE)
  ...
  for (const file of sourceFiles) {
    ...
    if (!response.ok) throw new Error(`Source download failed with HTTP ${response.status}: ${file.virtualPath}`)
    if (!response.body) throw new Error(`Source download has no body: ${file.virtualPath}`)
    const result = await hashReadableStream(response.body, pieceSize, file.size, maxBytes, torrentHasher)
    const comment = String(fileComments[file.relativePath] ?? fileComments[file.virtualPath] ?? "")
    files.push(applyHashMatrix({
  ...
  return {
    ...
    seed: normalizeSeed({
      name: String(body.name || (files.length === 1 ? files[0].path.split("/").pop() : "OpenList Seed")),
...
function chooseSource(c: any, file: SeedFile): { source: SeedSource; url: URL } | undefined {
...
async function uploadStreamToDriver(
  ...
  try {
    if (typeof dynamic.rapidUpload === "function") {
      const result = await dynamic.rapidUpload(actualTarget, resolved.physical, metadata)
      if (result === true || result?.reuse || result?.success) return { status: "reused", method: "rapidUpload" }
    }
    if (typeof dynamic.putRapid === "function") {
      const result = await dynamic.putRapid(actualTarget, resolved.physical, metadata)
      if (result === true || result?.reuse || result?.success) return { status: "reused", method: "putRapid" }
    }
    ...
    if (typeof dynamic.createUploadSession === "function") {
      ...
      if (uploadSession?.reuse) return { status: "reused", method: "multipart-rapid" }
    }
    if (rapidOnly) throw new Error("capability unavailable: target driver did not reuse the available hashes")
    if (!source) throw new Error("capability unavailable: no rapid-upload match and no approved source URL")
    ...
    if (uploadSession?.session && typeof dynamic.uploadPart === "function" && typeof dynamic.completeUploadSession === "function") {
      ...
      while (true) {
        ...
        if (value?.length) {
          ...
          if (transferred > file.size) throw new Error("Source exceeds the declared file size")
          ...
          while (pending.length >= chunkSize) {
            ...
            const result = await dynamic.uploadPart(uploadSession.session, partNumber++, Buffer.from(part))
            if (result?.partMd5) partMd5s.push(result.partMd5)
          }
        }
        if (done) break
      }
      if (transferred !== file.size) throw new Error("Source size does not match seed metadata")
      if (pending.length || file.size === 0) {
        const result = await dynamic.uploadPart(uploadSession.session, partNumber, Buffer.from(pending))
        if (result?.partMd5) partMd5s.push(result.partMd5)
      }
    ...
    if (typeof dynamic.putStream === "function") {
      await dynamic.putStream(actualTarget, resolved.physical, response.body, file.size, metadata)
      return { status: "saved", method: "putStream" }
    }

    const bufferedMax = envNumber(c, "SEED_BUFFERED_TRANSFER_MAX", DEFAULT_BUFFERED_TRANSFER_MAX)
    if (file.size <= bufferedMax) {
      const bytes = new Uint8Array(await response.arrayBuffer())
      if (bytes.byteLength !== file.size) throw new Error("Source size does not match seed metadata")
      await driver.put(actualTarget, resolved.physical!, Buffer.from(bytes))
      return { status: "saved", method: "bounded-buffer" }
    }
    throw new Error("capability unavailable: target driver has no streaming or multipart upload API")
  } finally {
    await flushPendingDriverState(resolved.storage.driver, resolved.storage, driver, storageContext(c))
  }
}
...
seedRouter.get("/capabilities", async (c) => {
    ...
    try {
      ...
      if (!resolved.isVirtual && resolved.storage) {
        ...
        target = {
          ...
          rapid_upload: typeof dynamic.rapidUpload === "function" || typeof dynamic.putRapid === "function" || typeof dynamic.createUploadSession === "function",
          stream_upload: typeof dynamic.putStream === "function" || (typeof dynamic.createUploadSession === "function" && typeof dynamic.uploadPart === "function" && typeof dynamic.completeUploadSession === "function"),
        }
      }
    } catch (error) {
  ...
  return c.json({
    ...
    data: {
      ...
      queue_adapter: false,
      limits: {
        metadata_bytes: envNumber(c, "SEED_MAX_METADATA_SIZE", DEFAULT_MAX_SEED_BYTES),
...
seedRouter.post("/capabilities", async (c) => {
  ...
  try {
    ...
    if (resolved.isVirtual || !resolved.storage) throw new Error("Target storage was not found")
    const driver = await getDriver(resolved.storage.driver, resolved.storage)
    const dynamic = driver as any
    const rapid =
    ...
    const files = parsed.seed.files.map((file) => {
      const hasRapidHash = !!file.hashes.md5 || !!file.hashes.sha1 || !!file.hashes.sha256
      ...
      return { path: file.path, method, requires_download: method === "download_required" }
    })
    return c.json({
      code: 200,
      message: "success",
...
seedRouter.post("/generate", async (c) => {
  ...
  try {
    const body = await readJson(c)
    if ((body.save_path || body.output_path) && !canWrite(user)) return errorResponse(c, 403, "Permission denied")
    const requestedFormats: unknown[] = Array.isArray(body.formats) ? body.formats : [body.format || "oss"]
    ...
    for (const format of formats) {
      ...
      const bytes = format === "torrent" ? await encodeTorrent(seed, torrentPieces) : await encodeSeed(seed, format)
      const outputDirectory = body.output_path || (Array.isArray(body.formats) ? body.save_path : undefined)
      const savePath = outputDirectory
        ? joinVirtualPath(normalizeVirtualPath(outputDirectory), fileNameFor(seed, format))
        : body.save_path
      await saveEncodedSeed(c, user, savePath, bytes)
      outputs.push(encodedResult(seed, format, bytes, {
        convertible: true,
    ...
    return c.json({
      code: 200,
      message: "success",
      data: Array.isArray(body.formats) ? { seed, outputs, artifacts: outputs } : outputs[0],
    })
  } catch (error) {
    const message = safeErrorMessage(error)
    return errorResponse(c, message.includes("limit") || message.includes("exceeds") ? 413 : 400, message)
  }
})
...
seedRouter.post("/parse", async (c) => {
  ...
  try {
    const body = await readJson(c)
    const parsed = await parseBodySeed(c, user, body)
    const totalSize = parsed.seed.files.reduce((sum, file) => sum + file.size, 0)
    const hasSource = parsed.seed.files.some((file) => !!chooseSource(c, file))
    const hasRapidHashes = parsed.seed.files.some((file) =>
...
seedRouter.post("/convert", async (c) => {
  ...
  try {
    ...
    const target = detectFormat(body.to_format || body.target_format || body.output_format)
    if (!target) throw new Error("A valid target format is required")
    const parsed = await parseBodySeed(c, user, body)
    const diagnostics = conversionDiagnostics(parsed.seed, target)
    if (diagnostics.length) {
      return c.json({ code: 200, message: "success", data: { convertible: false, diagnostics } })
    }
...
async function transferHandler(c: any, rapidOnly: boolean) {
  ...
  try {
    const body = await readJson(c)
    const parsed = await parseBodySeed(c, user, body)
    const targetRoot = normalizeVirtualPath(body.target_path || body.destination || body.path || "/")
    const selected = new Set<string>(
      Array.isArray(body.files) ? body.files.map((path: unknown) => String(path)) : [],
    )
    ...
    for (const file of files) {
      ...
      try {
        if (!rapidOnly && !source) {
          throw new Error("capability unavailable: offline download requires an approved source URL")
        }
...
seedRouter.post("/update", async (c) => {
  ...
  try {
    ...
    await saveEncodedSeed(c, user, body.save_path, bytes)
    return c.json({ code: 200, message: "success", data: encodedResult(seed, format, bytes) })
  } catch (error) {
    return errorResponse(c, 400, safeErrorMessage(error))
  }
})
