...
s3Router.get("/status", async (c) => {
  ...
  const s3AccessKeyId =
    db.settings.find((s: any) => s.key === "s3_access_key_id")?.value || ""
  const s3BucketsRaw =
    db.settings.find((s: any) => s.key === "s3_buckets")?.value || "[]"
...
s3Router.get("/:bucket", async (c) => {
  ...
  if (!bucket) {
    c.status(404)
    return c.text(
      `<?xml version="1.0" encoding="UTF-8"?><Error><Code>NoSuchBucket</Code><Message>The specified bucket does not exist</Message><BucketName>${bucketName}</BucketName></Error>`,
      404,
      {
